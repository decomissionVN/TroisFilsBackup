...
function language_used(){
    ...
    if (language === 'fr'){
        let translation = {
            ...
            title_order : "Toutes vos commandes",
            ...
            Finalize : "Payer la garantie",
            Follow_order : "Suivre ma commande",
            Payment_Valid : "Votre payement a bien été pris en compte!",
            Confection : "En cours de confection",
            Ready : "Solder ma commande",
            on_way : "Suivre mon colis",
            delivered : "Recommander",
        } 
        display_footer(translation);
        insert_the_HTML(translation);
        User_logged(translation);
        User_exists(translation);
        payment_carried_out(translation);
    } else {
        let translation = {
            ...
            Payment_Valid : "Your payment have been saved successfully",
            Confection : "Follow the confection",
            Ready : "Finalize the payment",
            on_way : "Follow my parcel",
...
function payment_carried_out(translation) {
    let queryParams = new URLSearchParams(window.location.search);
    let Payment = queryParams.get('Payment');
        
    if (Payment === '200'){
            console.log('ok200');
            document.getElementById('green_success').style.display = "block";
            document.getElementById('Payment_Valid').innerHTML = translation.Payment_Valid;
        }
}
